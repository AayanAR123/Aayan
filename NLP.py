...
def index():
    ...
    if request.method == 'POST':
        ...
        if new_file:
            ...
            for cluster in grouped_requirements.index:
                ...
                if new:
                    for l_req in legacy:
                        for n_req in new:
                            l_req, n_req, combined_sentence = create_combined_sentence(l_req, n_req)
                            if combined_sentence:
                                integrated_requirements.append({
                                    'legacy': l_req,
                                    'new': n_req,
                                    'integrated': combined_sentence.strip()
                                })
                    if len(legacy) == 0:
                        for n_req in new:
                            integrated_requirements.append({
        ...
        output_df = pd.DataFrame(integrated_requirements)
        output_df.to_csv(output_csv_path, index=False)

    # Load integrated requirements from the CSV file
    output_csv_path = os.path.join(app.config['UPLOAD_FOLDER'], 'integrated_requirements.csv')
    if os.path.exists(output_csv_path):
        integrated_requirements = pd.read_csv(output_csv_path).to_dict(orient='records')
